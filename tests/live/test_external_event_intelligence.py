...
a signed escalation webhook (a CI-escalation demo shape) lands on the AUT
gateway's ``/webhook`` asking it to phone a specific contact — the driver — and
we verify the agent actually *places that call* to the driver's number. The
driver sits on ``auto_reject``: we only care that the agent reasoned
"escalation → call this contact" and dialed; we do not handle the call.
...
SIGNING_KEY = os.environ.get("CODEX_INKBOX_SIGNING_KEY") or os.environ.get("INKBOX_SIGNING_KEY")
...
def _accepted(status: int, body: str) -> bool:
    """Whether the gateway accepted the webhook as an external event."""
    if status != 200:
        return False
    try:
...
def ctx():
    ...
    _first_phone(aut)  # the AUT must own a number to place the call from
...
def test_external_escalation_makes_agent_call_driver(ctx):
    ...
    envelope = {
        ...
        "severity": "prod",
        "summary": "Deploy failed on main; production is down.",
        "requested_action": (
        ...
        "github": {
            ...
            "workflow": "Deploy",
            "run_id": run_id,
            "run_url": f"https://github.com/example-org/backend/actions/runs/{run_id}",
        },
    }

    status, body = _post_external_event(envelope)
    assert _accepted(status, body), f"webhook not accepted: {status} {body!r}"
    ...
    pytest.fail(f"agent never called the driver within {TIMEOUT_S:.0f}s")
